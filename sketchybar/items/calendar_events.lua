...
--
-- Split out of calendar.lua so `cal.upcoming` is CREATED BEFORE the system
-- status widgets (bar order = item-creation order within a region). The
-- dynamic-width countdown therefore opens the right region in its own
-- capsule, and the fixed-width clock (items/calendar.lua, created last)
-- anchors the far right. The events popup is hosted on the "cal" item and
-- built from here on click (see items/calendar.lua).
...
-- F2: upcoming-events popup (hosted on the "cal" item)
function M.build_events_popup()
