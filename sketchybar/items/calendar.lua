...
--
-- Layout note: this item must be created FIRST among the right-region items
-- (require order in items/init.lua): sketchybar lays out right-positioned
-- items in reverse creation order, so the clock anchors the far right.
-- The upcoming-events countdown lives in items/calendar_events.lua and is
-- created later (leftmost of the right region).
--
-- Clicking the date toggles the month-grid calendar popup (restored from the
-- pre-00b8e8c implementation).
-- ---------------------------------------------------------------------------
...
M.cal:subscribe({ "forced", "routine", "system_woke" }, function()
...
M.cal:subscribe("mouse.clicked", function(env)
	...
	if drawing ~= "on" then
		build_calendar() -- rebuild on every open so the month/today marker is fresh
	end
