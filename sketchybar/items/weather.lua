...
os.execute("mkdir -p " .. cache_dir) -- Ensure the cache directory exists

local popup_width = 200

local function get_location()
	local handle = io.popen(location_script)
	local result = handle:read("*a") -- Read command output
	handle:close()
	return result:gsub("\n", "")
end
...
local forecast_script = string.format(
	...
	cache_dir, -- Pass in the cache directory path
	location
)
			...
			sbar.exec(forecast_script, function(forecast_data)
				if not forecast_data or forecast_data == "" then
					M.weather_forecast:set({
						label = { string = "Unable to fetch future weather" },
					})
					return
				end
